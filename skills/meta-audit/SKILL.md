...
model: opus
description: Audit Opencode subagent configs for completeness, security, and best practices. Use when reviewing agents/ for missing frontmatter, overprivileged tools, or bad model choices.
args: '[--verbose]'
...
| --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| Reviewing existing `~/.config/opencode/agents/**/*.md` files for missing frontmatter, overprivileged tools, or wrong models | Authoring a new agent definition file from scratch                        |
...
- Agent definitions: !`find . -path '~/.config/opencode/agents/*' -name "*.md" -not -name "settings*"`
- Settings file: !`find . -path '$/.config/opencode/agents/*' -maxdepth 3 -name "settings.local.json" -type f`

## Your task

### 1. Discovery Phase

- Use **Glob** to find all agent definition files in `~/.config/opencode/agents/`
...
- ✅ **model**: Opencode model to use (e.g., "opencode/deepseek-v4-flash-free")
- ✅ **color**: Hex color code for UI (e.g., "#E53E3E")
- ✅ **description**: Opencode usage guidance with "Use proactively when..."
